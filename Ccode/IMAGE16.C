...
// PORT AS NEEDED
typedef UINT16 InternalBucket;  // in Image16.c we store 1 pixel in a UINT16 sized bucket
...
static PolyMap* newPolyMap()
{
	...
	if (memBlk == NULL)
	  rb_raise(rb_eRuntimeError,"Image16 cannot find enough/correct memory configuration for internal structures (1.1)");
...
static void sizePolyMap(PolyMap* pmap, INT32 rows, INT32 cols)
{
	...
	if (pmap->rowsOfPolyIds == NULL)
	  rb_raise(rb_eRuntimeError,"Image16 cannot find enough/correct memory configuration for internal structures (2.1)");
    ...
    for (r = 0; r < rows; r++)
    {
	  ...
	  if (pmap->rowsOfPolyIds[r] == NULL)
	    rb_raise(rb_eRuntimeError,"Image16 cannot find enough/correct memory configuration for internal structures (2.2)");
    }
}
...
static VALUE cImage16 = Qnil;

static void freeRowsOfPixels(Image16* image)
{
  INT32 r;
...
static void Image16_free(Image16* image)
{
  if (image)
  {
...
inline
static void setBucket(Image16* i, INT32 r, INT32 c, InternalBucket b)
{
	i->rowsOfPixelBuckets[r][bucketNumber(c)] = b;
}

inline
static InternalBucket getBucket(Image16* i, INT32 r, INT32 c)
{
	return i->rowsOfPixelBuckets[r][bucketNumber(c)];
}

inline
static void imageSetP(Image16* i, INT32 r, INT32 c, Pixel pixValue)
{
  setBucket(i,r,c,internal(getBucket(i,r,c),sliceNumber(c),pixValue));
}

inline
static Pixel imageGetP(Image16* i, INT32 r, INT32 c)
{
...
static VALUE t_specify(VALUE self, VALUE rowsFixedNum, VALUE colsFixedNum)
{
  ...
  Data_Get_Struct(imageObj,Image16,image);
  ...
  if (image->rowsOfPixelBuckets == NULL)
    rb_raise(rb_eRuntimeError,"Image16 cannot find enough/correct memory configuration for internal structures (3.1)");
  ...
  for (r = 0; r < rows; r++)
  {
      ...
      if (image->rowsOfPixelBuckets[r] == NULL)
        rb_raise(rb_eRuntimeError,"Image16 cannot find enough/correct memory configuration for internal structures (3.2)");
  }
...
{
  Image16* image = calloc(1,sizeof(Image16));

  if (image == NULL)
    rb_raise(rb_eRuntimeError,"Image16 cannot find enough/correct memory configuration for internal structures (4)");

  rb_iv_set(self,"@image",Data_Wrap_Struct(cImage16,0,Image16_free,image));
...
static VALUE t_getCell(VALUE self, VALUE row, VALUE col)
{
  if (RTEST(self) && FIXNUM_P(row) && FIXNUM_P(col))
  {
    ...
    rb_raise(rb_eRuntimeError,"Image16.getCell() has received faulty arguments.");
...
static VALUE t_setCell(VALUE self, VALUE row, VALUE col, VALUE pixelValue)
{
  if (RTEST(self) && FIXNUM_P(row) && FIXNUM_P(col) && FIXNUM_P(pixelValue))
  {
    ...
    rb_raise(rb_eRuntimeError,"Image16.setCell() has received faulty arguments.");
...
static void makePolyMap(VALUE self, Image16* image, VALUE eightNeighbors)
{
  PolyMap* polyMap;
...
void Init_Image16(void)
{
  VALUE val = rb_require("SparseMatrix.rb");
  cImage16 = rb_define_class("Image16",rb_cObject);
  rb_define_method(cImage16, "initialize", t_init, 2);
  rb_define_method(cImage16, "specify", t_specify, 2);
  rb_define_method(cImage16, "getCell", t_getCell, 2);
  rb_define_method(cImage16, "setCell", t_setCell, 3);
  rb_define_method(cImage16, "rows", t_rows, 0);
  rb_define_method(cImage16, "cols", t_cols, 0);
  rb_define_method(cImage16, "classesPresent", t_classesPresent, 0);
  rb_define_method(cImage16, "legend", t_legend, 0);
  rb_define_method(cImage16, "palette", t_palette, 0);
  rb_define_method(cImage16, "title", t_title, 0);
  rb_define_method(cImage16, "title=", t_setTitle, 1);
  rb_define_method(cImage16, "each", t_each, 0);
  rb_define_method(cImage16, "each5", t_each5, 0);
  rb_define_method(cImage16, "each9", t_each9, 0);
  rb_define_method(cImage16, "polyCount", t_polyCount, 1);
  rb_define_method(cImage16, "polyAreas", t_polyAreas, 1);
  rb_define_method(cImage16, "polyPerims", t_polyPerims, 1);
  rb_define_method(cImage16, "polyClasses", t_polyClasses, 1);
  rb_define_method(cImage16, "area", t_area, 0);
  rb_define_method(cImage16, "perimeter", t_perimeter, 0);
  rb_define_method(cImage16, "areas", t_areas, 0);
  rb_define_method(cImage16, "perimeters", t_perimeters, 0);
  rb_define_method(cImage16, "pIJ", t_pIJ, 2);
}
