...
// PORT AS NEEDED
typedef BYTE InternalBucket;  // in Image1.c we store 8 pixels in a BYTE sized bucket
...
{
	return col / 2;  // divide by 2 : 2 pixels per bucket
}
...
{
	return col % 2;  // 0 to 1 : 2 pixels per bucket
}
...
{
    // slice num ranges from 0 to 1
	return (InternalBucket) ((currBucket & ~(15 << (sliceNum*4))) | (newPix << (sliceNum*4)));
}
...
{
    // slice num ranges from 0 to 1
	return (Pixel) ((bucket >> (sliceNum*4)) & 15);
}
...
static PolyMap* newPolyMap()
{
	...
	if (memBlk == NULL)
	  rb_raise(rb_eRuntimeError,"Image4 cannot find enough/correct memory configuration for internal structures (1.1)");
...
static void sizePolyMap(PolyMap* pmap, INT32 rows, INT32 cols)
{
	...
	if (pmap->rowsOfPolyIds == NULL)
	  rb_raise(rb_eRuntimeError,"Image4 cannot find enough/correct memory configuration for internal structures (2.1)");
    ...
    for (r = 0; r < rows; r++)
    {
	  ...
	  if (pmap->rowsOfPolyIds[r] == NULL)
	    rb_raise(rb_eRuntimeError,"Image4 cannot find enough/correct memory configuration for internal structures (2.2)");
    }
}
...
static VALUE cImage4 = Qnil;

static void freeRowsOfPixels(Image4* image)
{
  INT32 r;
...
static void Image4_free(Image4* image)
{
  if (image)
  {
...
inline
static void setBucket(Image4* i, INT32 r, INT32 c, InternalBucket b)
{
	i->rowsOfPixelBuckets[r][bucketNumber(c)] = b;
}

inline
static InternalBucket getBucket(Image4* i, INT32 r, INT32 c)
{
	return i->rowsOfPixelBuckets[r][bucketNumber(c)];
}

inline
static void imageSetP(Image4* i, INT32 r, INT32 c, Pixel pixValue)
{
  setBucket(i,r,c,internal(getBucket(i,r,c),sliceNumber(c),pixValue));
}

inline
static Pixel imageGetP(Image4* i, INT32 r, INT32 c)
{
...
static VALUE t_specify(VALUE self, VALUE rowsFixedNum, VALUE colsFixedNum)
{
  ...
  Data_Get_Struct(imageObj,Image4,image);
  ...
  if (image->rowsOfPixelBuckets == NULL)
    rb_raise(rb_eRuntimeError,"Image4 cannot find enough/correct memory configuration for internal structures (3.1)");
  ...
  for (r = 0; r < rows; r++)
  {
      ...
      if (image->rowsOfPixelBuckets[r] == NULL)
        rb_raise(rb_eRuntimeError,"Image4 cannot find enough/correct memory configuration for internal structures (3.2)");
  }
...
{
  Image4* image = calloc(1,sizeof(Image4));

  if (image == NULL)
    rb_raise(rb_eRuntimeError,"Image4 cannot find enough/correct memory configuration for internal structures (4)");

  rb_iv_set(self,"@image",Data_Wrap_Struct(cImage4,0,Image4_free,image));
...
static VALUE t_getCell(VALUE self, VALUE row, VALUE col)
{
  if (RTEST(self) && FIXNUM_P(row) && FIXNUM_P(col))
  {
    ...
    rb_raise(rb_eRuntimeError,"Image4.getCell() has received faulty arguments.");
...
static VALUE t_setCell(VALUE self, VALUE row, VALUE col, VALUE pixelValue)
{
  if (RTEST(self) && FIXNUM_P(row) && FIXNUM_P(col) && FIXNUM_P(pixelValue))
  {
    ...
    rb_raise(rb_eRuntimeError,"Image4.setCell() has received faulty arguments.");
...
static void makePolyMap(VALUE self, Image4* image, VALUE eightNeighbors)
{
  PolyMap* polyMap;
...
void Init_Image4(void)
{
  VALUE val = rb_require("SparseMatrix.rb");
  cImage4 = rb_define_class("Image4",rb_cObject);
  rb_define_method(cImage4, "initialize", t_init, 2);
  rb_define_method(cImage4, "specify", t_specify, 2);
  rb_define_method(cImage4, "getCell", t_getCell, 2);
  rb_define_method(cImage4, "setCell", t_setCell, 3);
  rb_define_method(cImage4, "rows", t_rows, 0);
  rb_define_method(cImage4, "cols", t_cols, 0);
  rb_define_method(cImage4, "classesPresent", t_classesPresent, 0);
  rb_define_method(cImage4, "legend", t_legend, 0);
  rb_define_method(cImage4, "palette", t_palette, 0);
  rb_define_method(cImage4, "title", t_title, 0);
  rb_define_method(cImage4, "title=", t_setTitle, 1);
  rb_define_method(cImage4, "each", t_each, 0);
  rb_define_method(cImage4, "each5", t_each5, 0);
  rb_define_method(cImage4, "each9", t_each9, 0);
  rb_define_method(cImage4, "polyCount", t_polyCount, 1);
  rb_define_method(cImage4, "polyAreas", t_polyAreas, 1);
  rb_define_method(cImage4, "polyPerims", t_polyPerims, 1);
  rb_define_method(cImage4, "polyClasses", t_polyClasses, 1);
  rb_define_method(cImage4, "area", t_area, 0);
  rb_define_method(cImage4, "perimeter", t_perimeter, 0);
  rb_define_method(cImage4, "areas", t_areas, 0);
  rb_define_method(cImage4, "perimeters", t_perimeters, 0);
  rb_define_method(cImage4, "pIJ", t_pIJ, 2);
}
