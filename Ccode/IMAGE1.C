...
{
	return col / 8;  // divide by 8 : 8 pixels per bucket
}
...
{
	return col % 8;  // 0 to 7 : 8 pixels per bucket
}
...
{
    // slice num ranges from 0 to 7
	return (InternalBucket) ((currBucket & ~(1 << sliceNum)) | (newPix << sliceNum));
}
...
{
    // slice num ranges from 0 to 7
	return (Pixel) ((bucket >> sliceNum) & 1);
}
...
static PolyMap* newPolyMap()
{
	...
	if (memBlk == NULL)
	  rb_raise(rb_eRuntimeError,"Image1 cannot find enough/correct memory configuration for internal structures (1.1)");
...
static void sizePolyMap(PolyMap* pmap, INT32 rows, INT32 cols)
{
	...
	if (pmap->rowsOfPolyIds == NULL)
	  rb_raise(rb_eRuntimeError,"Image1 cannot find enough/correct memory configuration for internal structures (2.1)");
    ...
    for (r = 0; r < rows; r++)
    {
	  ...
	  if (pmap->rowsOfPolyIds[r] == NULL)
	    rb_raise(rb_eRuntimeError,"Image1 cannot find enough/correct memory configuration for internal structures (2.2)");
    }
}
...
static VALUE cImage1 = Qnil;

static void freeRowsOfPixels(Image1* image)
{
  INT32 r;
...
static void Image1_free(Image1* image)
{
  if (image)
  {
...
inline
static void setBucket(Image1* i, INT32 r, INT32 c, InternalBucket b)
{
	i->rowsOfPixelBuckets[r][bucketNumber(c)] = b;
}

inline
static InternalBucket getBucket(Image1* i, INT32 r, INT32 c)
{
	return i->rowsOfPixelBuckets[r][bucketNumber(c)];
}

inline
static void imageSetP(Image1* i, INT32 r, INT32 c, Pixel pixValue)
{
  setBucket(i,r,c,internal(getBucket(i,r,c),sliceNumber(c),pixValue));
}

inline
static Pixel imageGetP(Image1* i, INT32 r, INT32 c)
{
...
static VALUE t_specify(VALUE self, VALUE rowsFixedNum, VALUE colsFixedNum)
{
  ...
  Data_Get_Struct(imageObj,Image1,image);
  ...
  if (image->rowsOfPixelBuckets == NULL)
    rb_raise(rb_eRuntimeError,"Image1 cannot find enough/correct memory configuration for internal structures (3.1)");
  ...
  for (r = 0; r < rows; r++)
  {
      ...
      if (image->rowsOfPixelBuckets[r] == NULL)
        rb_raise(rb_eRuntimeError,"Image1 cannot find enough/correct memory configuration for internal structures (3.2)");
  }
...
{
  Image1* image = calloc(1,sizeof(Image1));

  if (image == NULL)
    rb_raise(rb_eRuntimeError,"Image1 cannot find enough/correct memory configuration for internal structures (4)");

  rb_iv_set(self,"@image",Data_Wrap_Struct(cImage1,0,Image1_free,image));
...
static VALUE t_getCell(VALUE self, VALUE row, VALUE col)
{
  if (RTEST(self) && FIXNUM_P(row) && FIXNUM_P(col))
  {
    ...
    rb_raise(rb_eRuntimeError,"Image1.getCell() has received faulty arguments.");
...
static VALUE t_setCell(VALUE self, VALUE row, VALUE col, VALUE pixelValue)
{
  if (RTEST(self) && FIXNUM_P(row) && FIXNUM_P(col) && FIXNUM_P(pixelValue))
  {
    ...
    rb_raise(rb_eRuntimeError,"Image1.setCell() has received faulty arguments.");
...
static void makePolyMap(VALUE self, Image1* image, VALUE eightNeighbors)
{
  PolyMap* polyMap;
...
void Init_Image1(void)
{
  VALUE val = rb_require("SparseMatrix.rb");
  cImage1 = rb_define_class("Image1",rb_cObject);
  rb_define_method(cImage1, "initialize", t_init, 2);
  rb_define_method(cImage1, "specify", t_specify, 2);
  rb_define_method(cImage1, "getCell", t_getCell, 2);
  rb_define_method(cImage1, "setCell", t_setCell, 3);
  rb_define_method(cImage1, "rows", t_rows, 0);
  rb_define_method(cImage1, "cols", t_cols, 0);
  rb_define_method(cImage1, "classesPresent", t_classesPresent, 0);
  rb_define_method(cImage1, "legend", t_legend, 0);
  rb_define_method(cImage1, "palette", t_palette, 0);
  rb_define_method(cImage1, "title", t_title, 0);
  rb_define_method(cImage1, "title=", t_setTitle, 1);
  rb_define_method(cImage1, "each", t_each, 0);
  rb_define_method(cImage1, "each5", t_each5, 0);
  rb_define_method(cImage1, "each9", t_each9, 0);
  rb_define_method(cImage1, "polyCount", t_polyCount, 1);
  rb_define_method(cImage1, "polyAreas", t_polyAreas, 1);
  rb_define_method(cImage1, "polyPerims", t_polyPerims, 1);
  rb_define_method(cImage1, "polyClasses", t_polyClasses, 1);
  rb_define_method(cImage1, "area", t_area, 0);
  rb_define_method(cImage1, "perimeter", t_perimeter, 0);
  rb_define_method(cImage1, "areas", t_areas, 0);
  rb_define_method(cImage1, "perimeters", t_perimeters, 0);
  rb_define_method(cImage1, "pIJ", t_pIJ, 2);
}
