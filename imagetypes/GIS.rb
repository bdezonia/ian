...
class FileHeader
  def initialize(binReader)
    ...
    # print "MAPTYP = ",@maptype," IAUTYP = ",@iautyp,"\n"
  end
...
class ImageConverter
  ...
  def help(verbose)
    if verbose
      ["GIS - ERDAS 7.4 compatible GIS file",
       "",
       "  To invoke from the command line type igis(file) or igis(path~file)",
       "  The file specification can contain wildcards such as *.gis"
      ]
    else
      ["GIS - ERDAS GIS"]
    end
  ...
  def readImage(engine,imageArgs,imageInstance)
    ...
    # for a GIS file image args should be 1 or 2: optional path and file
    ...
    if not FileTest.exists?(fileName)
      extPresent = false
      extPresent = true if fileName =~ /.*\.gis$/i
      fileName += ".gis" if not extPresent
      if not FileTest.exists?(fileName)
   ...
   #    like map.specify(rows,cols,Map::UPPER_LEFT)
    ...
    extPos = fileName.downcase.rindex(".gis")
