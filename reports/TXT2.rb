...
class ReportWriter
  ...
  def name
    "Text format by class (.TXT)"
  end
  
  def help(verbose)
    if verbose
      ["RTXT2 - Text report (by class)",
       "",
  ...
  def run(engine,options,image,analysesOutput,args,verbose)
    ...
    if image.nil?
      engine.error("TXT2 report writer did not receive an image to process")
      return nil
    end
    ...
    case args.length
      ...
      when 2
        ...
        engine.error("Too many arguments to TXT2 report.")
        return
    end
    ...
    if imageMeasures
      analysesOutput.each do | analysisOutput |
        if analysisOutput.outType == AnalysisType::IMAGE
          ...
          outfile.print " ",unit.abbrev if unit.abbrev.length > 0
          outfile.print "\n"
        end
      end
    end
    if classMeasures
      outfile.print "\nClass measures\n"
      classes = Hash.new
      image.file.areas.each_index do |index|
        classes[index] = index if index != backGround and image.file.areas[index]
      end
      listOfClasses = classes.to_a.sort
      listOfClasses.each do | kvpair |
        color = kvpair[0]
        ...
        analysesOutput.each do | analysisOutput |
          if analysisOutput.outType == AnalysisType::CLASS
            ...
            tail = name.rindex("by class")
            name = name[0,tail] if tail
            printf(outfile,"    %-48s : ", name)
            value = analysisOutput.output[kvpair[0]]
            if value.nil?
              printf(outfile,"%-12d",0)
